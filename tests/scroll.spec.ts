...
test.describe('Scroll Directive', () => {
    test('should support bidirectional scrolling', async ({ page }) => {
        await page.goto('http://localhost:4183/debug-scroll')
        ...
        const xInput = page.locator('input').nth(0)
        const yInput = page.locator('input').nth(1)
        ...
        await expect(yInput).toHaveValue('300')
